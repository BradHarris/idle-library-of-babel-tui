...
import type { StatsPanelProps, LatestPagePanelProps, WorkersPanelProps, LogPanelProps, StorageScalePanelProps, SearchOverlayProps } from './types.js';
import type { ReactNode } from 'react';

/**
 * StatsPanel — displays total pages, money, and pages/sec.
 */
export function StatsPanel({ pagesGenerated, currentPage, money, pagesPerSecond, tickRateLevel, tickRateCost }: StatsPanelProps): ReactNode {
  ...
  return (
    <Box flexDirection="column" borderColor="blue" borderStyle="single" paddingX={1}>
      ...
      <Box>
        ...
        {tickRateLevel > 0 && (
          // @ts-expect-error Ink Text `dim` prop not in type definitions (chalk modifier)
          <Text dim>(level {formatNumber(tickRateLevel)})</Text>
        )}
      </Box>
      <Box>
 ...
 */
export function LatestPagePanel({ currentPage, latestPage, wrapWidth = 70 }: LatestPagePanelProps): ReactNode {
 ...
 */
export function WorkersPanel({ playerWorkers, doublingMultipliers, doublingProgress, canAfford }: WorkersPanelProps): ReactNode {
  return (
 ...
 */
export function LogPanel({ log }: LogPanelProps): ReactNode {
 ...
 */
export function StorageScalePanel({ pagesGenerated }: StorageScalePanelProps): ReactNode {
 ...
 */
export function SearchOverlay({ searchQuery, searchResult, inputValue }: SearchOverlayProps): ReactNode {
  return (
