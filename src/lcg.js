 ...
 * Every possible 280-character string using the 43-character alphabet.
 */
export const LCG_M = 43n ** 280n;

/**
 * LCG multiplier: a = 44
 */
export const LCG_A = 44n;

/**
 * LCG increment: c = 1
 */
export const LCG_C = 1n;
 ...
 *
 * @param {bigint} base  — the base of the geometric series (e.g., 44n)
 * @param {bigint} n     — number of terms (non-negative)
 * @param {bigint} mod   — modulus for all operations
 * @returns {{ sum: bigint, pow: bigint }}
 */
export function geoPair(base, n, mod) {
 ...
 *       = geoPair(a, n, m).sum
 *
 * @param {bigint} pageNum — the iteration count (page number)
 * @returns {bigint} the LCG state X_pageNum
 */
export function lcgState(pageNum) {
 ...
 *
 * @param {bigint} state — the LCG state value
 * @param {string} alphabet — the 43-character alphabet
 * @returns {string} 280-character page string
 */
export function stateToPage(state, alphabet) {
  const chars = new Array(280);
  let temp = state;
  for (let i = 279; i >= 0; i--) {
...
/**
 * Generate a deterministic page string from a page number using the LCG.
 *
 * @param {bigint} pageNum — the page number to generate
 * @param {string} alphabet — the 43-character alphabet
 * @returns {string} — 280-character page string
 */
export function generatePage(pageNum, alphabet) {
  const state = lcgState(pageNum);
  return stateToPage(state, alphabet);
}
