...
export function App(): ReactNode {
  ...
  // Handle worker hire
  const handleHire = useCallback((tierId: string): void => {
    useGameStore.getState().hire(tierId);
  }, []);

  // Handle tick rate upgrade purchase
  const handleTickRateUpgrade = useCallback((): void => {
  ...
  useInput((input, key) => {
    ...
    const keyMap: Record<string, string> = { '1': 'writer', '2': 'manager', '3': 'overseer', '4': 'rector', '5': 'cardinal', '6': 'pope', '7': 'archbishop' };
