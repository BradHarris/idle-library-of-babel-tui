...
### Requirement: Mutable state mutations in game.js
**Reason**: Replaced by immutable Zustand store with `produce` middleware. The `tick()` and `purchaseWorker()` functions that mutated state in place are removed. A pure `tick(deltaMs)` function replaces them.
**Migration**: All tick and purchase logic moves to the Zustand store actions. Callers pass `deltaMs` to `tick()`.

### Requirement: Dual mutable/immutable pattern
**Reason**: The existence of `purchaseWorker` (mutating) and `purchaseWorkerPure` (immutable) created confusion. The store provides a single immutable interface.
**Migration**: `game.js` is removed entirely. All logic is in `gameStore.ts`.
