...
The system SHALL generate a 280-character page string deterministically from a page number. The page content is generated by converting the page number directly to a 280-digit base-43 number, mapping digits to the 43-character alphabet, and reversing the result before returning. (The LCG permutation layer is temporarily disabled as of 2026-05-12; see `lcg-page-generation` spec.) The same page number always produces the same (reversed) page string.
...
- **THEN** the returned string equals `stateToPage(0n, ALPHABET).split('').reverse().join('')` (i.e., the direct base-43 page reversed; LCG is currently bypassed)
