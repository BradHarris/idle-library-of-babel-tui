# worker-doubling Specification

## Purpose

Each worker tier gains an output multiplier that doubles at escalating purchase thresholds. This creates meaningful progression milestones where output jumps at 10, 20, 40, 80, 160, 320... purchased workers. The multiplier applies to total worker count (including cascade) for the tier, creating synergistic growth between player-bought progress and cascade effects.
...
- **WHEN** the player has purchased 10 writers (multiplier = 2) and has higher-tier workers cascading to add 10 more writers (20 total)
- **THEN** the effective writer output is 40 pages per second (20 total × 2 multiplier)
