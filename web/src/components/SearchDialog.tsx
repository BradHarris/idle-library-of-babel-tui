...
import type { SearchResult } from '@game/types';
...
export function SearchDialog() {
  ...
  const handleSearch = (): void => {
    ...
    if (address <= pagesGenerated) {
      ...
      setResult({ type: 'notFound', address, pagesGenerated });
    }
  };
          ...
          {result && result.type === 'notFound' && (
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-400 font-mono">{formatPageNumber(result.pagesGenerated)}</span>
              </div>
            </div>
          )}
