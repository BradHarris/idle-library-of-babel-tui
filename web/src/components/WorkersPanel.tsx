 ...
 */
function getBulkCost(tier: Tier, playerBoughtCount: number, count: number): number {
  let total = 0;
 ...
 */
function getMaxAffordable(tier: Tier, playerBoughtCount: number, money: number): number {
  let count = 0;
  let total = 0;
  while (true) {
...
export function WorkersPanel() {
  ...
  const [hireMode, setHireMode] = useState<'one' | 'next' | 'max'>('one');

  const thresholds = useMemo(() => getDoublingThresholds(), []);

  const handleHire = (tierId: string): void => {
